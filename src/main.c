...
void dumpdata(char* buf, int* buf_counter, int bytes, long address)//long addr, int bytes
{
    long parity = getparity(address);
    // 0x81 clear error flags
    ...
    {
        swdSetWriteMode();
        swdWriteBits(address,32); // start of address to dump,
...
void swdTrySWDJ(void)
{
    ...
    while(numDumps--)
    {
        printf("===== DUMP: %d ======\n", numDumps);
        swdinitialop();
        dumpdata(buf1, &buf_counter, bytes, addr);
        sleep_ms(1000);
    }
...
{ 
    ...
    //DETECTION LOGIC
    for (int i = 0; i < sizeof(data_buf) / sizeof(data_buf[0]); i++) {
